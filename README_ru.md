...
[![GitHub Repo](https://img.shields.io/badge/GitHub-Repo-black.svg?logo=github)](https://github.com/agentscope-ai/Orchestrator)
...
[![Last Commit](https://img.shields.io/github/last-commit/agentscope-ai/Orchestrator)](https://github.com/agentscope-ai/Orchestrator)
[![License](https://img.shields.io/badge/license-Apache%202.0-red.svg?logo=apache&label=License)](LICENSE)
[![Code Style](https://img.shields.io/badge/code%20style-black-black.svg?logo=python&label=CodeStyle)](https://github.com/psf/black)
[![GitHub Stars](https://img.shields.io/github/stars/agentscope-ai/Orchestrator?style=flat&logo=github&color=yellow&label=Stars)](https://github.com/agentscope-ai/Orchestrator/stargazers)
[![GitHub Forks](https://img.shields.io/github/forks/agentscope-ai/Orchestrator?style=flat&logo=github&color=purple&label=Forks)](https://github.com/agentscope-ai/Orchestrator/network)
...
<p align="center">
  <img src="https://gw.alicdn.com/imgextra/i1/O1CN01sens5C1TuwioeGexL_!!6000000002443-55-tps-771-132.svg" alt="Orchestrator Logo" width="120">
</p>
...
>
> **Расширение через Skills** — Встроенное планирование по расписанию, обработка PDF и Office, дайджест новостей и многое другое; пользовательские навыки подгружаются автоматически, без привязки к одному поставщику. Именно Skills определяют, что умеет Orchestrator.
>
...
>
> **Любые каналы** — DingTalk, Feishu, WeChat, Discord, Telegram и другие. Один Orchestrator, подключайтесь по необходимости.
>
> <details>
> <summary><b>Что можно делать с Orchestrator</b></summary>
>
> <br>
>
...
[2026-04-12] **CoPaw официально переходит на бренд Orchestrator.** Этот ребрендинг — важный шаг в следующую фазу разработки с открытым исходным кодом.
...
С сегодняшнего дня Orchestrator остаётся с вами и растёт вместе с вами.
...
> - **🐍 Я не хочу устанавливать Python**: [Десктопное приложение](#десктопное-приложение-beta) или [Установка через скрипт](#установка-через-скрипт) или [ModelScope Studio](https://modelscope.cn/studios/fork?target=AgentScope/Orchestrator)
...
- [Почему Orchestrator?](#почему-orchestrator)
...
Не требуется ручная настройка Python, одна команда автоматически завершает установку. Скрипт автоматически загрузит uv (менеджер пакетов Python), создаст виртуальное окружение, установит Orchestrator и все зависимости (включая Node.js и frontend-ресурсы). Примечание: может не работать в некоторых сетевых окружениях или при корпоративном контроле доступа.
...
>      - Проверьте доступность `uv`: Введите `uv --version` в CMD. Если отображается номер версии, **настройте только путь Orchestrator**. Если появляется сообщение `'uv' не является внутренней или внешней командой, исполняемой программой или пакетным файлом`, настройте оба пути.
...
>      - Путь Orchestrator: Обычно находится в `%USERPROFILE%\.qwenpaw\bin`.
...
>   - **Повторно запустите установку**: Откройте новый терминал и снова выполните скрипт установки для завершения установки `Orchestrator`.
>   - **Настройте переменную окружения `Orchestrator`**: Добавьте `%USERPROFILE%\.qwenpaw\bin` в системную переменную `Path`.
...
>
> Внутри Docker-контейнера `localhost` указывает на сам контейнер, а не на хост-машину. Если Ollama (или другие модельные сервисы) запущены на хосте, можно обеспечить доступ Orchestrator из контейнера следующими способами:
>
...
>   agentscope/qwenpaw:latest
> ```
> Затем в Orchestrator **Настройки → Модели** измените Base URL на `http://host.docker.internal:<порт>` — например, для Ollama используйте `http://host.docker.internal:11434`, для LM Studio — `http://host.docker.internal:1234/v1`.
>
...
Если вы хотите развернуть Orchestrator на Alibaba Cloud, используйте развертывание в один клик на ECS: откройте [ссылку развертывания Orchestrator на Alibaba Cloud ECS](https://computenest.console.aliyun.com/service/instance/create/cn-hangzhou?type=user&ServiceId=service-1ed84201799f40879884) и следуйте инструкциям на странице. Подробные шаги см. в [Сообщество разработчиков Alibaba Cloud: Развертывание вашего AI-ассистента за 3 минуты](https://developer.aliyun.com/article/1713682).

---

### Вариант 5: Использование ModelScope

**Не хотите локальную установку?** Используйте [ModelScope Studio](https://modelscope.cn/studios/fork?target=AgentScope/Orchestrator) для облачной настройки в один клик. Установите свою студию как **непубличную**, иначе другие смогут управлять вашим Orchestrator.
...
Если вы не привыкли использовать командную строку, можете загрузить и использовать десктопную версию Orchestrator без ручной настройки Python-окружения или выполнения команд.

#### Загрузка

Загрузите десктопное приложение с [GitHub Releases](https://github.com/agentscope-ai/Orchestrator/releases):
- **Windows**: `Orchestrator-Setup-<version>.exe`
- **macOS**: `Orchestrator-<version>-macOS.zip` (рекомендуется для Apple Silicon)
...
Когда вы загружаете приложение Orchestrator для macOS из Releases, macOS может отобразить: *"Apple не может проверить, что 'Orchestrator' не содержит вредоносного ПО"*. Это происходит потому, что приложение не нотаризовано. Вы все равно можете открыть его следующими способами:

- **Открытие через правый клик (рекомендуется)**
  Щелкните правой кнопкой (или Control + щелчок) по приложению Orchestrator → **"Открыть"** → в диалоговом окне снова нажмите **"Открыть"**. Это сообщит Gatekeeper, что вы доверяете приложению; после этого можно запускать его двойным щелчком как обычно.

- **Разрешить в системных настройках**
  Если всё ещё заблокировано, перейдите в **Системные настройки → Конфиденциальность и безопасность**, прокрутите до сообщения типа *"'Orchestrator' заблокирован, так как невозможно проверить разработчика"*, и нажмите **"Всё равно открыть"** или **"Разрешить"**.
...
Если вы используете **облачный API для LLM** (например, Tongyi Qianwen, Gemini, OpenAI), перед началом диалога необходимо настроить API Key. До настройки действительного ключа Orchestrator не будет нормально работать. Подробности см. в [официальной документации](https://qwenpaw.agentscope.io/docs/models#%E9%85%8D%E7%BD%AE%E4%BA%91%E6%8F%90%E4%BE%9B%E5%95%86).
...
Orchestrator может полностью запускать LLM на вашей машине без API-ключей и облачных сервисов. Подробности см. в [официальной документации](https://qwenpaw.agentscope.io/docs/models).
...
| Тема                                                          | Описание                                        |
| ------------------------------------------------------------- | ----------------------------------------------- |
| [Введение](https://qwenpaw.agentscope.io/docs/intro)            | Что такое Orchestrator и как его использовать         |
...
Orchestrator включает многоуровневые механизмы безопасности для защиты ваших данных и системы:
...
Поставьте звезду Orchestrator на GitHub, чтобы получать уведомления о новых релизах первыми.
...
| **Пользовательские модели Orchestrator**             | Поддержка мультимодальных моделей                                                                           | Запланировано        |
...
|                                                 | Протокол агентов: Orchestrator → Orchestrator                                                                          | В работе             |
|                                                 | Протокол агентов: OpenClaw → Orchestrator                                                                        | Запланировано        |
...
Orchestrator развивается через открытую совместную работу, и мы приветствуем любые формы участия! Ознакомьтесь с [Дорожной картой](#дорожная-карта) выше (особенно с пунктами, отмеченными как **Ищем участников**), чтобы найти интересующие вас области, и прочитайте [CONTRIBUTING](https://github.com/agentscope-ai/Orchestrator/blob/main/CONTRIBUTING.md), чтобы начать. Мы особенно приветствуем:
...
Присоединяйтесь к обсуждениям на [GitHub Discussions](https://github.com/agentscope-ai/Orchestrator/discussions), чтобы обсуждать идеи или браться за задачи.

---

## Почему Orchestrator?

Orchestrator расшифровывается как **Qwen Personal Agent Workstation** (персональная рабочая станция агентов Qwen), а также воплощает мудрость Qwen и теплоту Paw. Мы хотим, чтобы это был не холодный инструмент, а умная и тёплая «маленькая лапка», всегда готовая прийти на помощь — самый надёжный партнёр в вашей цифровой жизни.
...
Orchestrator собирает **анонимные** данные об использовании при выполнении `qwenpaw init`, чтобы помочь нам понять пользовательскую среду и оптимизировать продукт. Данные собираются **один раз на версию** — когда вы обновляете Orchestrator, происходит повторный сбор для отслеживания распределения версий.

**Собираемая информация:**

- Версия Orchestrator (например, 0.0.7)
...
При интерактивном запуске `qwenpaw init` вас спросят о согласии. При использовании режима `--defaults` согласие дается автоматически. Запрос появляется только один раз на версию и не влияет на функциональность Orchestrator.

---

## Лицензия

Orchestrator выпускается под лицензией [Apache License 2.0](LICENSE).

---

## Участники

Благодарим всех, кто внес вклад в развитие Orchestrator:

<a href="https://github.com/agentscope-ai/Orchestrator/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=agentscope-ai/Orchestrator" alt="Участники" />
</a>
