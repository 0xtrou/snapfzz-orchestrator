...
In Orchestrator, **heartbeat** means: on a fixed interval, ask Orchestrator the
“questions” you wrote in a file, and optionally send the Orchestrator’s reply to
**the channel where you last chatted**. Good for “regular check-ins, daily
digests, scheduled reminders” — Orchestrator runs without you sending a
message.
...
1. In the current agent’s workspace there is a **heartbeat query file** (default
   ...
   content is **what to ask Orchestrator on each run** (one or more paragraphs; Orchestrator
...
2. When **`enabled` is true** in config, the system runs on your **every**
   ...
   the user message → Orchestrator replies.
3. **Whether the reply goes to a channel** is set by **target**:
   - **main** — Run Orchestrator only; don’t send the reply to any channel (e.g. local
     self-check, logs).
   - **last** — Send the reply to the **channel/session where you last talked
     to Orchestrator** (e.g. if you last used DingTalk, the heartbeat reply goes to
     DingTalk).
...
Example (heartbeat on, Orchestrator only, no channel, every 30m) — in that agent’s
**`agent.json`**:
