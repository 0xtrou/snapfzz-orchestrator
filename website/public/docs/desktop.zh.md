# Orchestrator Desktop 桌面应用版使用指南
...
本文档说明如何在 Windows 和 macOS 系统上安装和使用 Orchestrator Desktop 桌面应用。
...
1. **下载安装包**
   从 [Release 页面][releases]下载 `Orchestrator-Setup-<version>.exe` 文件

2. **运行安装程序**
   双击 `.exe` 文件，按照安装向导提示完成安装
   - 默认安装位置：`C:\Users\<你的用户名>\AppData\Local\Orchestrator`
...
#### **Orchestrator Desktop** (推荐日常使用)

- **特点**: 静默启动，无终端窗口，界面简洁
- **适用场景**: 正常使用，不需要查看技术日志
- **启动方式**: 双击桌面或开始菜单的 "Orchestrator Desktop" 图标
- **技术说明**: 使用 VBScript 启动器，后台运行 Python 进程

#### **Orchestrator Desktop (Debug)** (调试模式)
...
- **适用场景**:
  ...
  - 开发测试
  - 报告 Bug 时需要提供日志
- **启动方式**: 双击开始菜单的 "Orchestrator Desktop (Debug)" 图标
- **日志内容**:
  - 应用启动信息
  - Python 错误堆栈
  - API 调用日志
...
A: 使用 "Orchestrator Desktop (Debug)" 模式启动，查看终端输出的错误信息

**Q: 如何卸载？**

A: 在 Windows 设置 → 应用 → 已安装的应用 → 找到 "Orchestrator Desktop" → 卸载
...
1. **下载压缩包**
   从 [Release 页面][releases]下载 `Orchestrator-<version>-macOS.zip` 文件

2. **解压缩**
   双击 `.zip` 文件自动解压，得到 `Orchestrator.app` 应用

3. **移动到应用程序文件夹 (可选)**
   将 `Orchestrator.app` 拖到 `/Applications` 文件夹

### 首次启动：解除系统安全限制

#### 为什么需要手动信任？

Orchestrator 应用**未经过 Apple 开发者签名和公证（Notarization）**，macOS Gatekeeper 会默认阻止运行。
...
1. **右键点击**（或 Control + 点击）`Orchestrator.app`
2. 在菜单中选择 **"打开"**
...
1. 打开 **系统设置 → 隐私与安全性**
2. 向下滚动，找到类似以下提示：
   _"已阻止使用 'Orchestrator'，因为无法验证开发者"_
3. 点击 **"仍要打开"** 或 **"允许"** 按钮
4. 输入管理员密码确认
...
```bash
# 切换到应用目录
cd /Applications  # 或您的 Orchestrator.app 所在目录
...
A: 将 `Orchestrator.app` 拖到废纸篓，然后删除 `~/.qwenpaw` 配置文件夹
...
- **GitHub Issues**: [提交问题](https://github.com/agentscope-ai/Orchestrator/issues)
