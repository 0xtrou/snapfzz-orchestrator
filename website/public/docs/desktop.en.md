# Orchestrator Desktop Application Guide
...
This guide explains how to install and use the Orchestrator Desktop application on Windows and macOS.
...
1. **Download the installer**
   Download `Orchestrator-Setup-<version>.exe` from the [Release page][releases]

2. **Run the installer**
   Double-click the `.exe` file and follow the installation wizard
   - Default installation location: `C:\Users\<your-username>\AppData\Local\Orchestrator`
...
#### **Orchestrator Desktop** (Recommended for daily use)
...
- **How to Launch**: Double-click the "Orchestrator Desktop" icon on desktop or Start Menu
- **Technical Note**: Uses VBScript launcher, runs Python process in background

#### **Orchestrator Desktop (Debug)** (Debug Mode)
...
- **How to Launch**: Double-click "Orchestrator Desktop (Debug)" icon in Start Menu
- **Log Contents**:
...
A: Use "Orchestrator Desktop (Debug)" mode to view terminal output for error messages

**Q: How to uninstall?**

A: Go to Windows Settings → Apps → Installed apps → Find "Orchestrator Desktop" → Uninstall
...
1. **Download the archive**
   Download `Orchestrator-<version>-macOS.zip` from the [Release page][releases]

2. **Extract**
   Double-click the `.zip` file to extract and get `Orchestrator.app`

3. **Move to Applications folder (Optional)**
   Drag `Orchestrator.app` to the `/Applications` folder

### First Launch: Bypassing System Security Restrictions

#### Why manual trust is needed?

Orchestrator is **not Apple Developer-signed or notarized**, so macOS Gatekeeper will block it by default.
...
1. **Right-click** (or Control + click) on `Orchestrator.app`
...
2. Scroll down to find a message like:
   _"'Orchestrator' was blocked from use because it cannot verify the developer"_
...
- Double-click `Orchestrator.app` to launch
...
cd /Applications  # or wherever your Orchestrator.app is located

# Set environment variables and launch (isolate packaged env, avoid conflicts)
APP_ENV="$(pwd)/Orchestrator.app/Contents/Resources/env"
...
A: Drag `Orchestrator.app` to the Trash, then delete the `~/.qwenpaw` configuration folder
...
- **GitHub Issues**: [Submit an issue](https://github.com/agentscope-ai/Orchestrator/issues)
