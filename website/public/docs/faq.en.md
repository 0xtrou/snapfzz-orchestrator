...
### How to update Orchestrator

To update Orchestrator, use the method matching your installation type:
...
5. If using the Desktop app (Windows `.exe` or macOS `.zip`), you currently need to uninstall and reinstall:
   - Uninstall Orchestrator on your machine
   - Download the latest build: https://qwenpaw.agentscope.io/downloads
   - Reinstall

After upgrading, restart the service with `qwenpaw app`.

If you previously used CoPaw, upgrading to Orchestrator only requires downloading the latest Orchestrator. No extra migration is needed; your configuration, memory, skills, and other data from the CoPaw era continue to work.

### How to initialize and start Orchestrator service
...
with Orchestrator's default port **8088**. This affects all installation methods
...
- Orchestrator fails to start, or browser cannot connect to `http://127.0.0.1:8088/`
...
Orchestrator is open source. Official repository:
`https://github.com/agentscope-ai/Orchestrator`
...
or Orchestrator GitHub [Releases](https://github.com/agentscope-ai/Orchestrator/releases).
...
### How to use Orchestrator-Flash series models

Orchestrator-Flash is a family of models tuned by the Orchestrator team for Orchestrator's core
...
Orchestrator-Flash models are currently open-sourced on
...
All built-in local providers in Orchestrator can be used with Orchestrator-Flash models:

#### Orchestrator Local (llama.cpp)

In the Orchestrator Local model interface, simply choose a Orchestrator-Flash model to
...
> Orchestrator Local is still in beta. Compatibility across different devices and
> runtime stability are still being improved. If you run into issues while
> using it, please open an issue on GitHub.
> If Orchestrator Local does not work properly in your environment, we recommend
> deploying Orchestrator-Flash with Ollama or LM Studio first.

#### Ollama

1. Download a quantized Orchestrator-Flash model from
   ...
   [Orchestrator-Flash-4B-Q4_K_M](https://www.modelscope.cn/models/AgentScope/Orchestrator-Flash-4B-Q4_K_M).

   - Download with ModelScope CLI:

     ```bash
     modelscope download --model AgentScope/Orchestrator-Flash-4B-Q4_K_M README.md --local_dir ./dir
     ```
...
the Orchestrator-Flash model repository you downloaded:
...
4. In Orchestrator model settings, choose the Ollama provider, then automatically load
   the model on the Models page.

#### LM Studio

1. Follow step 1 in the Ollama section above to download an appropriate
   quantized Orchestrator-Flash model.
...
4. In Orchestrator model settings, choose the LM Studio provider, then automatically
   load the model on the Models page.

### When using models deployed with Ollama / LM Studio, why can't Orchestrator complete multi-turn interactions, complex tool calls, or remember earlier instructions?

In most cases, this is not a Orchestrator bug. The root cause is usually that the
model's context length is configured too small.

When you deploy a local model with Ollama or LM Studio, if the model's
`context length` is too low, Orchestrator may show problems such as:
...
- Before running Orchestrator, set the model's `context length` to **at least 32K**
- For more complex tasks, frequent tool calls, or longer conversations, you
  may need a value **higher than 32K**

> ⚠️ **Before running Orchestrator, you must set the context length to 32K or higher**
>
> For local models deployed with Ollama or LM Studio, Orchestrator typically needs a
...
The easiest way to create a cron job is to talk to Orchestrator in the channel where you want the results. For example, say: “Create a scheduled task that reminds me to drink water every five minutes.” You can then see the enabled job in Console.

If a scheduled task does not run as expected, try the following:

1. Confirm that the Orchestrator service is running.
...
6. To verify that the task was created and can run, click **Execute Now**. If it works, you should see the reply in the target channel. You can also ask Orchestrator: “Trigger the ‘drink water reminder’ task I just created.”
...
Orchestrator supports API keys obtained via DashScope Coding Plan. If it still fails,
please check:
...
[issue](https://github.com/agentscope-ai/Orchestrator/issues) in the Orchestrator GitHub
...
and also provide your current model provider, model name, and Orchestrator version.
