...
Orchestrator 上下文管理分为两条并行的 Offload 路径，共同解决上下文窗口有限的问题：
...
当上下文接近限制时，Orchestrator 会自动触发压缩，将旧对话浓缩为结构化摘要。
...
配置文件位于 `~/.qwenpaw/config.json` 中的 `agents.running` 部分：
