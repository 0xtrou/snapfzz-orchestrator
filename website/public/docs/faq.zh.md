...
### Orchestrator 与 OpenClaw 的功能对比

请查看 [对比](/docs/comparison) 页面了解详细的功能对比。

### Orchestrator如何安装

Orchestrator 支持多种安装方式，详情请见文档 [快速开始](https://qwenpaw.agentscope.io/docs/quickstart)：
...
要更新 Orchestrator 到最新版本，可根据你的安装方式选择对应方法：
...
原 CoPaw 用户升级 Orchestrator，下载最新的 Orchestrator 即可，无需做额外适配即可继续使用 CoPaw 时期的所有配置、记忆、技能等。

### Orchestrator服务如何启动及初始化
...
在 Windows 上，Hyper-V 和 WSL2 可能会保留某些端口范围，这可能与 Orchestrator 的默认端口 **8088** 冲突。此问题影响所有安装方式（pip 安装、脚本安装、Docker、桌面应用）。
...
- Orchestrator 无法启动，或浏览器无法访问 `http://127.0.0.1:8088/`
...
Orchestrator 已开源，官方仓库地址：
`https://github.com/agentscope-ai/Orchestrator`

### 最新版本升级内容如何查看

具体版本变更可在官网 [更新日志](https://qwenpaw.agentscope.io/release-notes/?lang=zh) 或 Orchestrator GitHub 仓库 [Releases](https://github.com/agentscope-ai/Orchestrator/releases) 中查看。
...
### 如何使用 Orchestrator-Flash 系列模型

Orchestrator-Flash 是 Orchestrator 官方根据 Orchestrator 的应用场景专门调优的系列模型，共有 2B, 4B 和 9B 三个版本，且每个版本除原始模型外还提供了 4 bit 和 8 bit 两种量化版本，适合不同的显存环境和性能需求。

Orchestrator-Flash 模型目前已经在 [ModelScope](https://www.modelscope.cn/organization/AgentScope?tab=model) 以及 [Hugging Face](https://huggingface.co/agentscope-ai/models) 上开源，你可以直接从这两个平台下载使用。

Orchestrator 内置的本地提供商均可接入 Orchestrator-Flash 模型：

**Orchestrator Local (llama.cpp)**

直接在 Orchestrator Local 的模型界面中选择下载 Orchestrator-Flash 模型并启动即可。

![Start Model](https://img.alicdn.com/imgextra/i2/O1CN01Nl0aQb1a3XqqosqAC_!!6000000003274-2-tps-1342-1682.png)

> Orchestrator Local 目前仍处于测试阶段，对不同设备的兼容性以及运行稳定性仍在持续优化中，如果你在使用过程中遇到任何问题，欢迎随时在 GitHub 上提 issue 反馈。
> 如果无法正常使用 Orchestrator Local，建议先使用 Ollama 或 LM Studio 部署 Orchestrator-Flash 模型。

**Ollama**:

1. 从 [ModelScope](https://www.modelscope.cn/organization/AgentScope?tab=model) 或 [Hugging Face](https://huggingface.co/agentscope-ai/models) 下载 Orchestrator-Flash 量化版模型，这些模型后缀为 `Q8_0` 或 `Q4_K_M`，例如 [Orchestrator-Flash-4B-Q4_K_M](https://www.modelscope.cn/models/AgentScope/Orchestrator-Flash-4B-Q4_K_M)。
...
创建一个包含以下内容的文本文件 `qwenpaw-flash.txt`，注意将 `/path/to/your/qwenpaw-xxx.gguf` 替换为你下载的 Orchestrator-Flash 模型仓库中 `.gguf` 文件的绝对路径：
...
4. 在 Orchestrator 的模型配置中选择 Ollama 提供商，并在模型页面中自动获取模型即可。

**LM Studio**:

1. 参考 Ollama 的步骤 1 下载合适的 Orchestrator-Flash 量化版模型。
...
4. 在 Orchestrator 的模型配置中选择 LM Studio 提供商，并在模型页面中自动获取模型即可。

### 使用 Ollama / LM Studio 部署的模型时，为什么 Orchestrator 无法完成多轮交互、复杂工具调用，或记不住之前的指令？

这类问题通常不是 Orchestrator 本身异常，而是**模型上下文长度配置过小**导致的。

当你使用 Ollama 或 LM Studio 部署本地模型时，如果模型的 `context length` 设置太低，Orchestrator 在以下场景中就可能表现异常：
...
- 运行 Orchestrator 前，请将模型的 `context length` 设置为**至少 32K**
- 如果任务较复杂、工具调用较多或对话轮次较长，实际可能需要设置到**高于 32K**

> ⚠️ **运行 Orchestrator 前必须将上下文长度设为 32K 以上**
>
> 对于 Ollama 和 LM Studio 部署的本地模型，如果要让 Orchestrator 正常完成多轮交互、复杂工具调用和长上下文任务，通常必须提供 **32K 或更高** 的上下文长度；在更复杂的场景下，可能还需要进一步提高。
>
...
最方便的定时任务创建方式是，在你想要获取定时任务返回结果的频道，与Orchestrator对话，让Orchestrator帮你创建一个定时任务。例如，可以直接与Orchestrator对话：“帮我创建一个定时任务，每隔五分钟提醒我喝水。”之后可以在控制台中看到状态为已启用的定时任务。

如果定时任务没有正常启动，可以按照以下几个步骤排查：

1. 首先确认 Orchestrator 服务是在正常运行中的。
...
6. 排查结束后，如果想确认一下定时任务是否创建成功，且能成功触发，可以点击 **立即执行**，若成功创建，则可在对应频道收到回复。或者也可以直接与 Orchestrator 对话：“帮我触发一下刚刚创建的提醒喝水定时任务”。
...
Orchestrator 支持百炼 Coding Plan 获取的 API key。如果仍报错，请重点检查：
...
为了加快修复与排查，共建良好社区生态，建议遇到报错时，首选在 Orchestrator 的 GitHub 仓库中提 [issue](https://github.com/agentscope-ai/Orchestrator/issues)，请附上完整报错信息，并上传错误详情文件。
...
请将后面的`/var/folders/.../qwenpaw_query_error_qzbx1mv1.json`文件一并上传，同时提供你当前的模型提供商、模型名和 Orchestrator 的具体版本。
