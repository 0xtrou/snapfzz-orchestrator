...
- **Check [Open Issues](https://github.com/agentscope-ai/Orchestrator/issues)** and any [Projects](https://github.com/agentscope-ai/Orchestrator/projects) or roadmap labels.
...
Orchestrator is designed to be **extensible**: you can add models, channels, Skills, and more. Below are the main contribution areas we care about.

---

### Adding New Models / Model Providers

Orchestrator supports multiple providers, including cloud providers (such as DashScope, ModelScope) and local providers (such as Ollama, LM Studio). We also welcome new model providers to enrich user choices.
...
If the above conditions are met, you can create a new Provider instance in `src/qwenpaw/providers/provider_manager.py` and register it in the `ProviderManager` class to make it a built-in provider in Orchestrator.
...
Channels are how Orchestrator talks to **DingTalk, Feishu, QQ, Discord, iMessage**, etc. You can add a new channel so Orchestrator can work with your favorite IM or bot platform.
...
**Skills** define what Orchestrator can do: cron, file reading, PDF/Office, news, browser, etc. We welcome **broadly useful** base skills (productivity, documents, communication, automation) that fit the majority of users.
...
- **Skills Hub:** Orchestrator supports importing skills from a community hub (e.g. ClawHub). If you want your skill to be installable via hub, follow the same `SKILL.md` + `references/`/`scripts/` layout and the hub’s packaging format.
...
Orchestrator aims to run on **Windows**, **Linux**, and **macOS**. Contributions that improve support on a specific platform are welcome.
...
- **MCP (Model Context Protocol):** Orchestrator supports runtime **MCP tool** discovery and hot-plug. Contributing new MCP servers or tools (or docs on how to attach them) helps users extend the agent without changing core code.
...
- **Discussions:** [GitHub Discussions](https://github.com/agentscope-ai/Orchestrator/discussions)
- **Bugs and features:** [GitHub Issues](https://github.com/agentscope-ai/Orchestrator/issues)
- **Community:** DingTalk group (see [README](README.md)) and [Discord](https://discord.gg/eYMpfnkG8h)

Thank you for contributing to Orchestrator. Your work helps make it a better assistant for everyone. 🐾
